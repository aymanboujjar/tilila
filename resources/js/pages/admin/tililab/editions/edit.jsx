...
export default function AdminTililabEditionsEdit({ edition }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        gallery_images: Array.isArray(edition?.gallery_images) ? edition.gallery_images : [],
        gallery_images_files: [],
        remove_gallery_images: [],
        winners: Array.isArray(edition?.winners) ? edition.winners.map((p) => ({ ...p, photo: null })) : [],
        jury: Array.isArray(edition?.jury) ? edition.jury.map((p) => ({ ...p, photo: null })) : [],
    });
