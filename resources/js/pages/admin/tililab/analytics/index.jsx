                ...
                <div className="flex flex-col gap-3 border-b border-border/60 pb-6 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                        <Button asChild variant="outline">
                            <Link href="/admin/tililab/participants">Participants</Link>
                        </Button>
                    </div>
                </div>

                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    <KpiCard label="Total participants" value={kpis?.total ?? 0} />
                    <KpiCard label="New (last 7 days)" value={kpis?.last7Days ?? 0} />
                    <KpiCard label="Countries (top 12)" value={countries.length} />
                </div>
                    ...
                    <div className="rounded-xl border border-border/70 bg-card p-4 shadow-sm sm:p-6">
                        ...
                        <div className="mt-4 h-64">
                            {countries.length === 0 ? (
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={[...countries].reverse()} layout="vertical">
                                        <CartesianGrid strokeDasharray="3 3" opacity={0.25} />
                                        <XAxis
                                            type="number"
                                            allowDecimals={false}
                                        ...
                                        />
                                        <Tooltip />
                                        <Bar dataKey="count" fill="#22c55e" radius={[6, 6, 6, 6]} />
                                    </BarChart>
                                </ResponsiveContainer>
                            )}
                        </div>
