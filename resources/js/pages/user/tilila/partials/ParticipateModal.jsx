...
function Field({ label, children, error }) {
    return (
        <div className="space-y-2">
            ...
            {children}
            {error ? <div className="text-xs text-alpha-danger">{error}</div> : null}
        </div>
    );
}
        ...
        post('/tilila/participate', {
            ...
            onError: () => {
                requestAnimationFrame(() => {
                    const el = document.getElementById('tilila-participate-scroll');
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-4xl max-h-[85vh] overflow-hidden p-0">
                <div className="bg-background/95 supports-backdrop-filter:bg-background/80 border-b border-border px-6 py-5 backdrop-blur">
                ...
                >
                    ...
                    <div className="sm:col-span-2">
                        <Field
                            label={<TransText en="Email" fr="E-mail" ar="البريد الإلكتروني" />}
                            error={errors.email}
                        >
                    ...
                    <Field label={<TransText en="Phone" fr="Téléphone" ar="الهاتف" />} error={errors.phone}>
                        <Input value={data.phone} onChange={(e) => setData('phone', e.target.value)} />
                    </Field>
                    <Field label={<TransText en="Country" fr="Pays" ar="البلد" />} error={errors.country}>
                        <Input value={data.country} onChange={(e) => setData('country', e.target.value)} placeholder="ma" />
                    </Field>

                    <div className="sm:col-span-2 grid grid-cols-1 gap-5 sm:grid-cols-2">
                        <Field
                            label={<TransText en="Organization" fr="Organisation" ar="المنظمة" />}
                            error={errors.organization}
                        >
                            <Input value={data.organization} onChange={(e) => setData('organization', e.target.value)} />
                        </Field>
                        <Field
                            label={<TransText en="Job title" fr="Poste" ar="المنصب" />}
                    ...
                    <div className="sm:col-span-2">
                        <Field
                            label={<TransText en="City" fr="Ville" ar="المدينة" />}
                            error={errors.city}
                        >
                    ...
                    <div className="sm:col-span-2">
                        <Field
                            label={<TransText en="Submission title" fr="Titre de la candidature" ar="عنوان المشاركة" />}
                            error={errors.submission_title}
                        >
                            <Input value={data.submission_title} onChange={(e) => setData('submission_title', e.target.value)} />
                        </Field>
                    </div>

                    <div className="sm:col-span-2">
                        <Field
                            label={<TransText en="Short description" fr="Description (courte)" ar="وصف مختصر" />}
                            error={errors.submission_description}
                        >
                            <textarea
                                className="border-input placeholder:text-muted-foreground flex min-h-[120px] w-full rounded-md border bg-transparent px-3 py-2 text-sm shadow-xs outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px]"
                                value={data.submission_description}
                                onChange={(e) => setData('submission_description', e.target.value)}
                            />
                        </Field>
                    </div>
                    ...
                    <div className="sm:col-span-2">
                        <Field
                            label={<TransText en="Submission link" fr="Lien de soumission" ar="رابط المشاركة" />}
                    ...
                    <div className="sm:col-span-2 rounded-xl bg-background p-4 ring-1 ring-border">
                        <div className="flex items-start gap-3">
                            <Checkbox
                                id="accepted_rules"
                                checked={Boolean(data.accepted_rules)}
                                onCheckedChange={(v) => setData('accepted_rules', Boolean(v))}
                            />
                            <div className="min-w-0">
                                <Label htmlFor="accepted_rules" className="text-sm font-semibold text-foreground">
                                    <TransText
                    ...
                    <div className="sm:col-span-2 flex items-center justify-end gap-3 pt-2">
                        ...
                        >
                            <TransText
                                en={processing ? 'Submitting…' : 'Submit participation'}
                                fr={processing ? 'Envoi…' : 'Envoyer la participation'}
                                ar={processing ? 'جارٍ الإرسال…' : 'إرسال المشاركة'}
                            />
                        </button>
                    </div>
                    </div>
