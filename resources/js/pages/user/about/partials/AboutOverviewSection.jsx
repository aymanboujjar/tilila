...
function Card({ title, body, href, cta }) {
    return (
        <article className="flex h-full flex-col rounded-xl border border-border/70 bg-twhite p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
            <h3 className="text-base font-extrabold text-beta-blue sm:text-lg">
                {title}
            </h3>
            <div className="mt-3 flex-1 text-sm leading-relaxed text-tgray">
                {body}
            </div>
            {href ? (
                <Link
                    href={href}
                    className="mt-5 inline-flex items-center gap-1.5 text-xs font-bold tracking-wide text-beta-blue uppercase transition hover:text-brand-light-purple"
                >
                    {cta}
